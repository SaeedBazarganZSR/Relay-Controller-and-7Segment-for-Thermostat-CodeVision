...
void Routine(void)
{
    if (TCNT0 >= 250)
    {
        ...
        TempCheckSecond++;
        if(TempCheckSecond > 150)
        {
            TempCheckSecond = 0;
            TempInit = TempSensor_Update();
            TempTarget = TempInit;
                    
            if((TempSetTarget - TempInit) <= 0)
            {
                Relay_TurnOff(Three);
                Relay_TurnOff(Two);
                Relay_TurnOff(One);
            ...
            else if((TempSetTarget - TempInit) > 0 && (TempSetTarget - TempInit) < (DiffTempSet + 1))
            {
                Relay_TurnOn(Fan);
                Relay_TurnOn(Three);
                Relay_TurnOff(Two);
            ...
            else if((TempSetTarget - TempInit) >= (DiffTempSet + 1) && (TempSetTarget - TempInit) < (DiffTempSet + 2))
            {
                Relay_TurnOn(Fan);
                Relay_TurnOn(Three);
                Relay_TurnOn(Two);
            ...
            else if((TempSetTarget - TempInit) >= (DiffTempSet + 2))
            {
                Relay_TurnOn(Fan);
                Relay_TurnOn(Three);
                Relay_TurnOn(Two);
    ...
    if(FanFlag == 1 && TurnOFF_FanSecond > 2)
    {
        Relay_TurnOff(Fan);
        FanFlag = 0;                    
        TurnOFF_FanSecond = 0;
...
void Keys_Update(void)
{
    ...
    {                 
        ...
        EEPROM_Write(TempSetTarget);
    }
    else if(PIND.3 == 0)    //On_Btn
    {
        PORTB.0 = 1;
        PORTB.1 = 1;
        PORTB.2 = 1;
        ...
        if(SettingCount > 15)
        {            
            for(ToggleWarning = 0; ToggleWarning < 10; ToggleWarning++)
            {                  
                Warning_On();
                delay_ms(2500);
                Warning_Off();
